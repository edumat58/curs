/* src/components/InfiniteMenu/InfiniteMenu.jsx */
...
class Geometry {
    ...
    get vertexData() {
        ...
        this.vertices.forEach(v => {
            arr.push(v.position[0], v.position[1], v.position[2]);
        });
    ...
    get normalData() {
        const arr = [];
        this.vertices.forEach(v => {
            arr.push(v.normal[0], v.normal[1], v.normal[2]);
        });
        return new Float32Array(arr);
    }
...
class InfiniteGridMenu {
    constructor(canvas, items, onActiveItemChange, onMovementChange, onInit, scale = 1.0) {
        ...
        this.SPHERE_RADIUS = 1.5; // Closer items (was 2)
    ...
    init(onInit) {
        const gl = this.canvas.getContext('webgl2', {
            antialias: true,
            alpha: true // Allow transparency so CSS background shows
        });
        if (!gl) {
    ...
    initTexture() {
        ...
        // Initialize with a 1x1 white pixel
        ...
        // Requires 'Material Icons' to be loaded in the page header
        ...
        Promise.all(
            this.items.map(
                item =>
                    new Promise(resolve => {
                        // If item has a specific image (like Kulturosfera), load it
                        // Even if it has an image, we might want to mask it or draw it differently
                        if (item.image && !item.icon) {
                            const img = new Image();
                            img.crossOrigin = 'anonymous';
                            img.onload = () => resolve({ type: 'image', img });
                            img.onerror = () => {
                            ...
                            // It's an icon item
                            resolve({ type: 'icon', icon: item.icon || 'help' });
                        }
                    })
            )
        ).then(results => {
            results.forEach((result, i) => {
                ...
                const r = cellSize / 2 - 10; // Padding

                // Draw Orange Circle Background
                ctx.beginPath();
                ctx.arc(cx, cy, r, 0, Math.PI * 2);
                ctx.fillStyle = '#e67e22'; // EduMat Orange
                ctx.fill();
                ...
                if (result.type === 'icon') {
                    ...
                    ctx.fillText(result.icon, cx, cy);
                } else if (result.type === 'image' && result.img) {
                    // Draw Image, faded
                    ctx.save();
                    ctx.globalAlpha = 0.6; // Fade
                    // Draw image to fit in circle
                    const imgAspect = result.img.width / result.img.height;
                    let drawWidth = cellSize * 0.6;
                    let drawHeight = drawWidth / imgAspect;
                    if (drawHeight > cellSize * 0.6) {
                        drawHeight = cellSize * 0.6;
                        drawWidth = drawHeight * imgAspect;
                    }
                    ctx.drawImage(result.img, cx - drawWidth / 2, cy - drawHeight / 2, drawWidth, drawHeight);
                    ctx.restore();
                }
            });
    ...
    animate(deltaTime) {
        ...
        // Auto-rotation
        if (!this.control.isPointerDown && !this.control.snapTargetDirection) {
            const autoRotSpeed = 0.0005 * deltaTime; // Speed
            const rotQ = quat.create();
            quat.setAxisAngle(rotQ, [0, 1, 0], autoRotSpeed); // Integrate rotation around Y axis
            quat.multiply(this.control.orientation, rotQ, this.control.orientation);
...
const InfiniteMenu = ({ items = [], scale = 1.0 }) => {
    ...
    useEffect(() => {
        ...
        if (canvas) {
            console.log('InfiniteMenu: Mounting with items:', items);
